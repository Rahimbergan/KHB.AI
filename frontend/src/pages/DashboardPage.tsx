...
export const DashboardPage: React.FC<DashboardPageProps> = ({ currentDate }) => {
  ...
  if (error || !report) {
    return (
      <div className="bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-300 p-4 rounded-xl text-sm">
  ...
  return (
    <div className="space-y-6 pb-12 transition-colors duration-200">
      {/* Top Banner / Welcome */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gradient-to-r dark:from-slate-900 dark:to-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((kpi, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm hover:border-slate-300 dark:hover:border-slate-700 transition-all"
            >
              ...
              <div className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">{kpi.value}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-semibold text-slate-900 dark:text-white">Daily Sales Trend (Month to Date)</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Revenue across daily sales batches</p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-slate-900 dark:text-white">Top Products Today</h3>
            <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Ranked by revenue</span>
          </div>
          <div className="space-y-3">
            {report.top_products.slice(0, 5).map((prod, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
              >
                <div className="truncate mr-2">
                  <div className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">{prod.product_name}</div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">{prod.category} · {prod.units} units</div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl space-y-3 shadow-sm">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Fastest moving categories today: <span className="text-slate-900 dark:text-white font-medium">Smart Home & Audio</span>.
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs font-semibold text-slate-800 dark:text-slate-300">Need Deep Business Advice?</div>
      ...
      <div className="text-[11px] text-slate-400 italic text-center pt-4 border-t border-slate-200 dark:border-slate-800">
        {report.disclaimer}
      </div>
    </div>
  );
