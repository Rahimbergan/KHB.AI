  ...
  return (
    <div className="space-y-6 pb-12 transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Sales Orders & Analytics</h2>
      ...
      <div className="flex items-center justify-between gap-4 bg-white dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-emerald-500"
          />
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 text-slate-600 dark:text-slate-400 font-semibold uppercase tracking-wider">
                ...
                salesData?.data.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="py-3.5 px-4 font-mono font-medium text-emerald-600 dark:text-emerald-400">{order.order_number}</td>
                      <td className="py-3.5 px-4 text-slate-600 dark:text-slate-300">
                        {order.order_date.replace('T', ' ').substring(0, 16)}
                      </td>
                      <td className="py-3.5 px-4 text-slate-800 dark:text-slate-200 font-medium">{order.customer_name}</td>
                      ...
                      <td className="py-3.5 px-4 text-slate-600 dark:text-slate-300 uppercase text-[11px] font-medium">
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 disabled:opacity-40 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700"
              >
                Previous
              </button>
              <button
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-3xl w-full p-6 space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 dark:hover:text-white text-sm px-2 py-1 font-semibold cursor-pointer"
              >
                ✕ Close
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/60">
                <div className="text-[11px] text-slate-500 dark:text-slate-400">Total Revenue</div>
                <div className="text-sm font-bold text-slate-900 dark:text-white mt-1">
                  {formatCurrency(analysisResult.total_revenue)}
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/60">
                <div className="text-[11px] text-slate-500 dark:text-slate-400">Gross Margin</div>
                <div className="text-sm font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                  {analysisResult.gross_margin}%
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/60">
                <div className="text-[11px] text-slate-500 dark:text-slate-400">Total Orders</div>
                <div className="text-sm font-bold text-slate-900 dark:text-white mt-1">
                  {analysisResult.number_of_orders}
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700/60">
                <div className="text-[11px] text-slate-500 dark:text-slate-400">Units Sold</div>
                <div className="text-sm font-bold text-slate-900 dark:text-white mt-1">
            ...
            {analysisResult.previous_period_comparison && (
              <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 p-4 rounded-xl space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {analysisResult.anomalies.map((a, i) => (
                    <div
                      key={i}
                      className="text-xs bg-amber-50 dark:bg-slate-800/70 border border-amber-200 dark:border-slate-700 p-2.5 rounded-lg text-slate-700 dark:text-slate-300"
                    >
                      {a.description}
                    </div>
                  ))}
            ...
            {/* Disclaimer */}
            <div className="text-[11px] text-slate-400 italic pt-2 border-t border-slate-200 dark:border-slate-800">
