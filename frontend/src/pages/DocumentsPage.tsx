  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Documents & Contract Intelligence</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
          <div className="text-sm font-bold text-white px-2">Uploaded Documents</div>
          {isLoading ? (
            <div className="text-xs text-slate-400 p-4 text-center">Loading documents...</div>
          ) : files?.length === 0 ? (
            <div className="text-xs text-slate-400 p-4 text-center">No documents uploaded yet.</div>
          ) : (
        ...
        <div className="lg:col-span-2 space-y-6">
          {selectedFile ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Type: <span className="uppercase font-semibold text-slate-300">{selectedFile.file_type}</span> · Size: {(selectedFile.file_size / 1024).toFixed(1)} KB
                  </p>
                </div>
              ...
              {analysis && (
                <div className="space-y-4 bg-slate-800/40 border border-slate-700/60 p-5 rounded-xl">
                  ...
                  {/* Summary */}
                  <div>
                    <div className="text-xs font-bold text-slate-300 mb-1">Executive Summary:</div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                      <div className="text-xs font-semibold text-slate-300 mb-1">Identified Parties:</div>
                    ...
                    <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                      <div className="text-xs font-semibold text-slate-300 mb-1">Monetary Terms:</div>
                      <ul className="text-xs text-slate-400 space-y-0.5 list-disc list-inside">
                  ...
                  {/* Risks */}
                  <div className="bg-slate-900/60 p-3 rounded-lg border border-rose-900/40 space-y-2">
                  ...
                  {/* Legal Disclaimer */}
                  <div className="text-[11px] text-slate-400 italic pt-2 border-t border-slate-700/60">
                    {analysis.disclaimer}
                  </div>
                </div>
              )}
              ...
              {/* Text Preview */}
              <div>
                <div className="text-xs font-semibold text-slate-400 mb-2">Content Preview:</div>
                <pre className="text-xs text-slate-300 bg-slate-950 p-4 rounded-xl border border-slate-800 max-h-72 overflow-y-auto whitespace-pre-wrap font-mono leading-relaxed">
